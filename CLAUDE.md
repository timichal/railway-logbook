...
- `npm run updateStationProximity` — recompute `stations.near_railway` (250m to the nearest railway part), adding the column if missing. `importMapData` does this automatically; run it to backfill a database loaded before the column existed.
...
- **stations** — Point features from OSM. `near_railway` BOOL — a railway part runs within 250m (`STATION_RAIL_PROXIMITY_METERS`), recomputed at the end of every `importMapData`. OSM holds plenty of station points nowhere near any track we keep, so the user map is served only the flagged ones (`public_stations_tile`); the admin map shows all (`stations_tile`). `searchStations` (map search box + Journey Planner autocomplete) and `inspectPath`'s name lookup are restricted to the same set, so nothing can be searched that isn't drawn; `inspectPath` still takes a numeric station id verbatim as the escape hatch.
...
- **Vector tiles** via Martin (port 3001): `railway_routes_tile` (accepts `selected_countries` filter), `railway_parts_tile` (zoom-filtered), `stations_tile` (zoom 9+, all stations — admin map), `public_stations_tile` (same, `near_railway` only — user map; both emit the MVT layer name `stations`, so one MapLibre layer definition serves either), `admin_notes_tile` (all notes, admin map), `public_notes_tile` (`note_type='Usage'` only, exposes just text+source — user map).
...
### Scripts (`src/scripts/`)
- **Data**: `pruneData.ts`, `importMapData.ts`, `verifyRouteData.ts`, `applyVectorTiles.ts`, `markAllRoutesInvalid.ts` (migration reference), `fixSequences.ts` (resync SERIAL sequences), `updateStationProximity.ts` (recompute `stations.near_railway`), `listStations.ts`, `inspectPath.ts` (journey-planner debug), `exportRoutes.ts`, `importRoutes.ts`.
