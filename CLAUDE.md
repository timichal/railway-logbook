...
- `npm run updateStationProximity` — recompute `stations.near_route` (250m to the nearest admin-defined route), adding the column if missing. The import/verify/route-write paths all do this themselves; run it to backfill.
...
### Scripts (`src/scripts/`)
- **Data**: `pruneData.ts`, `importMapData.ts`, `verifyRouteData.ts`, `applyVectorTiles.ts`, `markAllRoutesInvalid.ts` (migration reference), `fixSequences.ts` (resync SERIAL sequences), `updateStationProximity.ts` (recompute `stations.near_route`), `listStations.ts`, `inspectPath.ts` (journey-planner debug), `exportRoutes.ts`, `importRoutes.ts`.
