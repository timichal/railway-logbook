...
5. **Prune** (`pruneData.ts`) - Applies custom filters to remove unwanted features (subways, etc.)
...
8. **Database Load** (`load-data.js`) - Imports processed data into PostgreSQL with user separation
...
- **Tables**:
  ...
  - `stations` - Railway stations (Point features from OSM)
  - `railway_routes` - Railway lines with metadata (LineString features)
  - `user_railway_data` - User-specific ride history and notes
...
- **Leaflet** - Interactive mapping library for route visualization
...
- `load-data.js` - Database loading script (Node.js, not TypeScript)
