...
- **Two transports, one set of queries.** Every user-scoped query lives in a plain server-only module taking a `userId` — `progressQueries.ts`, `journeyQueries.ts`, `tripQueries.ts`, `preferencesQueries.ts`, plus the user-less `routeQueries.ts`, `authQueries.ts` and `routePathFinder.ts`. The `*Actions.ts` modules are thin: resolve the session, delegate. `publicMapActions.ts` resolves a share token instead, and the route handlers under `src/app/api/v1` resolve a bearer token — three callers, one query.

  The rule that keeps it honest: **a route handler never imports a `"use server"` module**, and the reason is the one `progressQueries.ts`'s header gives — every export of one is a client-callable endpoint, so a query taking a user id as an argument must not be exposed as an action. Where a needed query still sits inside an action, lift it out rather than importing the action.

  This is also why `authTokens.ts` (JWT minting, both transports' claim shape) and `authQueries.ts` (bcrypt, account creation) are separate from `authActions.ts`, which is now only the cookie. A `ValidationError` (`errors.ts`) is how a query says its message is meant for whoever asked: the web forms render `error.message` as they always did, the API turns it into a 400, and anything else stays an opaque 500 so a Postgres error text is never handed to a client.
- **The HTTP API (`/api/v1`)** exists for the native app and nothing else — see `API.md` for the endpoints and `MOBILE_APP_PLAN.md` for why. The web app keeps its server actions untouched, which makes it the regression test for anything changed underneath. Auth is a bearer token (`Authorization: Bearer …`), signed by the same secret as the web cookie; the cookie itself is never read by a handler. `region` is an explicit parameter everywhere it appears and **never defaults** — on the web it comes from a cookie, and over HTTP a missing one would mean a query answering for the other continent. Versioned from the first commit, because a binary in the wild cannot be asked to change.
...
**The colour expression avoids `["all", ...]`, and that is not a style preference.** `getUserRouteColorExpression` states its four visit cases as *nested single-condition* `case`s, and `lineClassColorExpression` is a `match` rather than a chain of `==` cases. Both read a little less directly than the flat forms they replaced, and both are that way because **MapLibre Native's React binding throws `std::bad_alloc` converting a `case` that carries an `all` condition alongside a further branch** — the shape this expression had. It is a hard crash at style-value conversion (`layer.lineColor = styleValue.mlnStyleValue`), with no JS error and no crash report; found by running the Phase 0 mobile spike on a device, and written up in `MOBILE_APP_PLAN.md` under "The colour expression". GL JS is indifferent between the forms, so the web app carries the one that also works on native instead of leaving a second implementation to be discovered later. Nesting loses nothing: reaching an inner `case` already means the outer condition held, so `has date` AND `has_complete_trip` needs no conjunction. **Keep new data-driven paint expressions in this shape** — one condition per `case`, `match` for a value fan-out.
...
**Park label points are filtered out of the park outline** (`filterPointsFromParkOutlines`). OpenMapTiles' `park` source-layer carries a label point per park alongside the polygon, and liberty's `park_outline` is an unfiltered `line` layer over it — so each of those points (dozens per tile from z6 up) is handed to a line bucket as a single-coordinate geometry. Nothing renders wrong, since a point cannot be drawn as a line, but **MapLibre Native logs `Invalid geometry in line layer`** for it where GL JS says nothing; that warning came out of the mobile spike, and it is the basemap's, not our route tile's — those decode as 100% LineString. The guard is only applied where the layer carries no filter of its own: liberty's filters are legacy-syntax, and a legacy filter cannot be `["all", ...]`-combined with an expression one.
...
- `api/v1/**/route.ts` — the native app's HTTP API (`API.md`). Each handler is thin: resolve the bearer token, validate the input, call a query module, map the result to a status.
...
### Library (`src/lib/`)
- **DB/queries** (plain server-only modules taking a `userId`; see "Two transports, one set of queries"): `db.ts`, `dbConfig.ts`, `progressQueries.ts`, `journeyQueries.ts`, `tripQueries.ts`, `preferencesQueries.ts`, `routeQueries.ts` (public station/route reads), `authQueries.ts` (bcrypt + account creation), `authTokens.ts` (JWT for cookie and bearer alike), `errors.ts` (`ValidationError`).
- **Actions** (`"use server"`, session-resolving wrappers): `userActions.ts`, `journeyActions.ts`, `tripActions.ts`, `userPreferencesActions.ts`, `authActions.ts` (the cookie), `plannerActions.ts`, `publicMapActions.ts` (token-resolving), `migrationActions.ts`, `adminRouteActions.ts`, `adminMapActions.ts`, `adminNotesActions.ts`.
- **API plumbing** (`src/lib/api/`): `auth.ts` (bearer parsing, token issue), `params.ts` (query-string and body validation), `response.ts` (status mapping, including how an in-band `{ error }` becomes a status code).
