...
async function importRoutes() {
  ...
  if (hasOldHslColumn) {
    console.log(
      "Detected old schema (hsl column) in dump file — will migrate to line_class after import",
    );
  }

  console.log(`Importing railway data from ${filename}...`);
  console.log(
    "This will import: railway_routes, user_journeys, user_logged_parts (user_id=1), and admin_notes\n",
  );
  ...
  try {
    ...
    try {
      ...
      try {
        ...
        const loggedPartsCount = await query(
          "SELECT COUNT(*) FROM user_logged_parts WHERE user_id = 1",
        );
