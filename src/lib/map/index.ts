 ...
 * web app's tile host to the source factories in `tileSources.ts`, which take it as
 * an argument so the native app can pass its own (see that file's header). So a web
 * call site still writes `createRailwayRoutesSource({ userId })` and never mentions
 * a URL.
 *
...
export { type RailwayRoutesSourceOptions, ZOOM_RANGES } from "./tileSources";
...
export const createRailwayRoutesSource = (options: sources.RailwayRoutesSourceOptions = {}) =>
  sources.createRailwayRoutesSource(TILE_BASE_URL, options);
